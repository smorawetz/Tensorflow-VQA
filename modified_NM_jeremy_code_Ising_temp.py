...
from modified_HelperFunctionsandDataGeneration_Ising import *
...
def getFreeEnergy(Nqubits, samples, log_probs, T, BoundaryListX, BulkListX, FullListX):
    """
    ...
    energy = SyndromeX(samples, Nqubits, BoundaryListX, BulkListX, FullListX)
...
def annealingSurfaceCode(
    ...
    results_path = "results/Ising_L{0}_results".format(Nqubits)
        ...
        with Errors.graph.as_default():
            ...
            for step in range(Nwarm + Nequilibrium * Nsteps + 1):
                ...
                if trainsteps > 0:
                    ...
                    # end = time.time()
                    # print("Done getting the output and log probs in {0} s".format(end - outtime))
                ...
                for viz_num in range(1, mid_anneal_num_viz + 1):
                    names_dict[Nwarm + viz_num * make_viz_step] = "mid_viz{0}_of{1}".format(viz_num, mid_anneal_num_viz)
                ...
                if step in names_dict.keys():
                    ...
                    fname_cost = "{0}/cost_landscape_viz_{1}_max{2}_Npoints{3}.txt".format(
                        savelocation, names_dict[step], max_change, N_POINTS
                    )
