...
for L in Ls:
    for T in Ts:
        ...
        for SEED in SEEDs:
            ...
            ax.plot(T, F / L ** 2, "k-", label="Neural Annealing Results")
            ...
            # if model_type == "Ising":
                # analytical = np.zeros(len(T))
                # for i in range(len(T)):
                    # val_at_T = integrate.nquad(integrand, [[0, 2 * np.pi], [0, 2 * np.pi]], args=(1 / (T[i] + 1e-12),))[0]
                    # # val_at_T = integrate.nquad(integrand, [[0, 2 * np.pi], [0, 2 * np.pi]])
                    # analytical[i] = val_at_T
                    # print(T[i])
                    # print(val_at_T)
                    # print(i)
                # ax.plot(T, analytical, "r-", label="Analytical Result")
            ...
            plot_name = (
                "{0}_L{1}_T{2}_nh{3}_lr{4}_Ns{5}_Nw{6}_Nt{7}_Na{8}_seed{9}_F_vs_T.png".format(
                    model_type, L, T0, NH, LR, NS, NW, NT, NA, SEED
                )
            )
            fig.legend(loc=[0.6, 0.82], frameon=False)
