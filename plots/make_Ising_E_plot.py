...
for L in Ls:
    for T in Ts:
        T0 = T
        for SEED in SEEDs:
            folder = "../ising_results/{0}_L{1}_results/L{1}_T{2}_nh{3}_lr{4}_Ns{5}_Nw{6}_Nt{7}_Na{8}_seed{9}".format(
            ...
            B_data = "exact_data/ising_B_data_L{0}.txt".format(L)
            energy_states_data = "exact_data/ising_energy_levels_L{0}.txt".format(L)

            B_data = np.loadtxt(B_data)
            energy_states_data = np.loadtxt(energy_states_data)

            states_to_plot = [0, 1, 4, 5]  # which energy states to plot

            colours = ["C{0}".format(x) for x in range(7)]
            ...
            colour_index = 0
            for state_index in states_to_plot:
                ax.plot(
                    B_data,
                    energy_states_data[:, state_index] / L ** 2,
                    colours[colour_index],
                    label=labels[colour_index],
                )
                colour_index += 1

            # ax.set_ylim(-1, 0)

            plot_name = "{0}_L{1}_T{2}_nh{3}_lr{4}_Ns{5}_Nw{6}_Nt{7}_Na{8}_seed{9}_E_vs_T.png".format(
                model_type, L, T0, NH, LR, NS, NW, NT, NA, SEED
            )
