...
# Ls = [3, 4, 5, 13, 16]  # Side length of lattice
...
Ls = [4, 5]
text_loc_str = [[0.78, -0.50], [0.78, -0.50]]
...
for T in Ts:
    ...
    for SEED in SEEDs:
        ...
        for i in [0, 1]:
            ...
            ax.set_xlabel(r"$B$")
            ax.set_ylabel(r"$F / L^2$" if i == 0 else "")

            # ax.set_title("Free energy density at different temperatures")

            ax.plot(T, F / L ** 2, "k-", label="Neural Annealing Results" if i == 1 else "")
            ax.text(*text_loc_str[i], "L = {0}".format(L), fontsize=14)
            ...
            ax.plot(
                ...
                "r-",
                label="Ground state energy" if i == 1 else "",
            )

        Ls_string = "{0}and{1}".format(Ls[0], Ls[1])

        fig.legend(loc=[0.16, 0.22], frameon=False)

        plot_name = (
            "plots_E/{0}_compare_L{1}_T{2}_nh{3}_lr{4}_Ns{5}_Nw{6}_Nt{7}_Na{8}_seed{9}_F_vs_T.png".format(
                model_type, Ls_string, T0, NH, LR, NS, NW, NT, NA, SEED
            )
        )
